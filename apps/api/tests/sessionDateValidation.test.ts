...
describe('isClinicalDateTooFarInFuture', () => {
  const now = Date.UTC(2026, 6, 19, 12, 0, 0);
  const at = (offset: number) => new Date(now + offset);

  it('la fecha de referencia no está en el futuro', () => {
    expect(isClinicalDateTooFarInFuture(at(0), now)).toBe(false);
  });

  it('un adelanto dentro del tope no se rechaza', () => {
    expect(isClinicalDateTooFarInFuture(at(APIFutureDateTolerance - 1), now)).toBe(false);
  });

  it('exactamente en el tope se permite (comparación estricta >)', () => {
    expect(isClinicalDateTooFarInFuture(at(APIFutureDateTolerance), now)).toBe(false);
  });

  it('un milisegundo más allá del tope se rechaza', () => {
    expect(isClinicalDateTooFarInFuture(at(APIFutureDateTolerance + 1), now)).toBe(true);
  });

  it('una fecha absurdamente futura se rechaza', () => {
    expect(isClinicalDateTooFarInFuture(at(400 * DAY_MS), now)).toBe(true);
  });

  it('el Date válido más grande (año 275760) se rechaza', () => {
    expect(isClinicalDateTooFarInFuture(new Date(8.64e15), now)).toBe(true);
  });

  it('una fecha pasada nunca dispara el tope de futuro', () => {
    expect(isClinicalDateTooFarInFuture(at(-400 * DAY_MS), now)).toBe(false);
  });

  it('una fecha pre-1970 (timestamp negativo) no es futuro', () => {
    expect(isClinicalDateTooFarInFuture(new Date('1900-01-01T00:00:00.000Z'), now)).toBe(false);
  });
  ...
  it('un Invalid Date se rechaza en vez de colarse', () => {
    expect(isClinicalDateTooFarInFuture(new Date('no-soy-fecha'), now)).toBe(true);
    expect(isClinicalDateTooFarInFuture(new Date(NaN), now)).toBe(true);
  });
});
...
describe('isClinicalDateBeforeFloor', () => {
  const floor = new Date(MIN_CLINICAL_DATE_MS);

  it('la fecha de hoy está por encima del piso', () => {
    expect(isClinicalDateBeforeFloor(new Date())).toBe(false);
  });

  // Lo que el tope viejo de 15 días rechazaba y ahora tiene que pasar.
  it('una sesión de hace meses se acepta', () => {
    expect(isClinicalDateBeforeFloor(new Date(Date.now() - 200 * DAY_MS))).toBe(false);
  });

  it('una sesión de hace años se acepta', () => {
    expect(isClinicalDateBeforeFloor(new Date('2015-03-04T09:00:00.000Z'))).toBe(false);
  });

  it('exactamente el piso se permite (comparación estricta <)', () => {
    expect(isClinicalDateBeforeFloor(floor)).toBe(false);
  });

  it('un milisegundo antes del piso se rechaza', () => {
    expect(isClinicalDateBeforeFloor(new Date(MIN_CLINICAL_DATE_MS - 1))).toBe(true);
  });

  it('una fecha pre-1970 (timestamp negativo) se rechaza', () => {
    expect(isClinicalDateBeforeFloor(new Date('1900-01-01T00:00:00.000Z'))).toBe(true);
  });

  it('un año tipeado mal se rechaza', () => {
    expect(isClinicalDateBeforeFloor(new Date('1025-07-19T12:00:00.000Z'))).toBe(true);
  });

  it('una fecha futura nunca dispara el piso', () => {
    expect(isClinicalDateBeforeFloor(new Date(Date.now() + 400 * DAY_MS))).toBe(false);
  });

  it('un Invalid Date se rechaza en vez de colarse', () => {
    expect(isClinicalDateBeforeFloor(new Date('no-soy-fecha'))).toBe(true);
    expect(isClinicalDateBeforeFloor(new Date(NaN))).toBe(true);
  });
});
