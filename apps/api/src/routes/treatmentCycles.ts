...
type Params = { patientId: string; episodeId: string; cycleId: string };

// Montado en /api/patients/:patientId/episodes/:episodeId/cycles
...
// GET /api/patients/:patientId/episodes/:episodeId/cycles
router.get<Pick<Params, 'patientId' | 'episodeId'>>('/', async (req, res) => {
  const data = await treatmentCycleRepo.list(DEV_CONTEXT, req.params.patientId, req.params.episodeId);
  res.json({ data });
});

// POST /api/patients/:patientId/episodes/:episodeId/cycles
router.post<Pick<Params, 'patientId' | 'episodeId'>>('/', async (req, res) => {
  const body = CycleCreateSchema.parse(req.body);
  const data = await treatmentCycleRepo.create(DEV_CONTEXT, req.params.patientId, {
    ...body,
...
// PATCH /api/patients/:patientId/episodes/:episodeId/cycles/:cycleId
...
// DELETE /api/patients/:patientId/episodes/:episodeId/cycles/:cycleId
